...
# ── Logging ─────────────────────────────────────────────────
ts()   { date '+%Y-%m-%d %H:%M:%S'; }
ok()   { echo -e "${G}  [✓]${N} $1"; echo "[$(ts)] OK   [$CURRENT_PHASE] $1" >> "$LOG"; PASS=$(( PASS + 1 )) || true; }
warn() { echo -e "${Y}  [!]${N} $1"; echo "[$(ts)] WARN [$CURRENT_PHASE] $1" >> "$LOG"; WARN=$(( WARN + 1 )) || true; }
fail() { echo -e "${R}  [✗]${N} $1"; echo "[$(ts)] FAIL [$CURRENT_PHASE] $1" >> "$LOG"; FAIL=$(( FAIL + 1 )) || true; }
...
preflight() {
    ...
    local heap_mb; heap_mb=$(( ram_kb / 1024 / 2 )) || true
    [[ "${heap_mb:-0}" -gt 31744 ]] && heap_mb=31744 || true
...
" 2>/dev/null; then
    ...
    [[ -f /var/ossec/etc/decoders/rh-pulsar-decoders.xml ]] && \
        { ok "Decoders: present"; p=$(( p + 1 )) || true; } || \
        { fail "Decoders: missing"; f=$(( f + 1 )) || true; }

    # Check rules loaded
    [[ -f /var/ossec/etc/rules/rh-pulsar-rules.xml ]] && \
        { ok "Rules: present"; p=$(( p + 1 )) || true; } || \
        { fail "Rules: missing"; f=$(( f + 1 )) || true; }

    # Check AR script
    [[ -f /var/ossec/active-response/bin/rh-pulsar-block.sh ]] && \
        { ok "Active response script: present"; p=$(( p + 1 )) || true; } || \
        { fail "Active response: missing"; f=$(( f + 1 )) || true; }

    # Check agent enrollment port
    ss -tlnp 2>/dev/null | grep -q ":1515 " && \
        { ok "Port 1515 (enrollment): listening"; p=$(( p + 1 )) || true; } || \
        { warn "Port 1515: not yet listening"; }

    ss -tlnp 2>/dev/null | grep -q ":1514 " && \
        { ok "Port 1514 (agent): listening"; p=$(( p + 1 )) || true; } || \
