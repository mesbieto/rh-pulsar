...
echo "  ╚═╝  ╚═╝╚═╝  ╚═╝    ╚═╝      ╚═════╝ ╚══════╝╚══════╝╚═╝  ╚═╝╚═╝  ╚═╝"
echo -e "${N}"
echo -e "${W}  Detection Validation — v3.2.0${N}"
...
if systemctl list-unit-files rh-pulsar-ja4-update.timer &>/dev/null; then
    ...
    if [[ -f /var/lib/rh-pulsar/ja4-update.state ]]; then
        last_status=$(grep "last_status=" /var/lib/rh-pulsar/ja4-update.state | cut -d= -f2)
        last_attempt=$(grep "last_attempt=" /var/lib/rh-pulsar/ja4-update.state | cut -d= -f2-)
        fp_count=$(grep "fingerprint_count=" /var/lib/rh-pulsar/ja4-update.state 2>/dev/null | cut -d= -f2)
        case "$last_status" in
            success)
                ok "Last refresh: ${last_attempt} (success)"
                ok "DB fingerprints loaded: ${fp_count:-?}" ;;
            *)
                warn "Last refresh: ${last_attempt} (status: ${last_status})"
