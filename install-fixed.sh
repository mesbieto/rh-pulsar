...
# /api/download/ = new bulk-download endpoint (listed first on ja4db.com homepage)
# /api/read/     = original JSON API (fallback)
API_DOWNLOAD_URL="https://ja4db.com/api/download/"
API_READ_URL="https://ja4db.com/api/read/"
...
# ── Embedded baseline — used when ALL API sources fail ─────
# These are publicly documented, IR-verified C2 fingerprints.
# RH team: update this list from your own threat intel.
# This guarantees non-zero DB coverage even without internet.
write_embedded_baseline() {
    cat > "$OUT_FILE" << 'BASELINE'
# RH Pulsar — Embedded JA4 Baseline (API unavailable fallback)
# Maintained by Red Horizon Security IR team
# DO NOT EDIT — managed by rh-pulsar-ja4-update.sh
module DetectJA4;

redef malicious_ja4_db += {
    # Sliver C2 (Go) — Red Horizon IR verified + John Althouse public
...
redef malicious_ja4s_db += {
    # Sliver C2 server response — Red Horizon IR verified
    "t130200_1301_a56c5b993250",
};

redef ja4_framework += {
    ["t13d190900_9dc949149365_97f8aa674fd9"] = "Sliver C2",
    ["t13i190900_9dc949149365_97f8aa674fd9"] = "Sliver C2",
    ["t13i3111h2_e8f1e7e78f70_b26ce05bbdd6"] = "Sliver C2 mTLS",
    ["t13i131000_f57a46bbacb6_e5728521abd4"] = "Sliver C2 pivot",
};
BASELINE
}

# ── Download: try /api/download/ first, then /api/read/ ────
# Why the custom UA was breaking things:
#   "RH-Pulsar/3.2.5" identifies us as an automated script —
#   ja4db.com rate-limits non-browser UAs aggressively.
#   Using a standard Chrome UA gets past that gate.
#
# Why 60s was too short:
#   The DB now has thousands of entries. Serialization + transfer
#   from certain regions (Southeast Asia especially) takes 90-180s.
#
# Why we check response size:
#   A rate-limit or error page is < 2KB. Real data is > 100KB.
#   jq type check alone won't catch an error page that happens
#   to contain valid (but tiny) JSON.
BROWSER_UA="Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Safari/537.36"
MIN_SIZE_BYTES=10000
DOWNLOAD_OK=false

# Attempt 1 — /api/download/ (new preferred endpoint)
log "Trying ja4db.com/api/download/ ..."
if curl -fsSL --compressed         --connect-timeout 15         --max-time 240         --retry 3 --retry-delay 10 --retry-max-time 300         -A "$BROWSER_UA"         "$API_DOWNLOAD_URL" -o "$TMP_JSON" 2>/dev/null; then
    SIZE=$(wc -c < "$TMP_JSON" 2>/dev/null || echo 0)
    if [[ "$SIZE" -ge "$MIN_SIZE_BYTES" ]] &&        jq -e 'type=="array"' "$TMP_JSON" >/dev/null 2>&1; then
        DOWNLOAD_OK=true
        log "api/download/: success (${SIZE} bytes)"
    else
        log "api/download/: response too small or wrong format (${SIZE} bytes) — trying api/read/"
    fi
fi

# Attempt 2 — /api/read/ (original endpoint)
if [[ "$DOWNLOAD_OK" == false ]]; then
    log "Trying ja4db.com/api/read/ ..."
    if curl -fsSL --compressed             --connect-timeout 15             --max-time 240             --retry 3 --retry-delay 15 --retry-max-time 300             -A "$BROWSER_UA"             "$API_READ_URL" -o "$TMP_JSON" 2>/dev/null; then
        SIZE=$(wc -c < "$TMP_JSON" 2>/dev/null || echo 0)
        if [[ "$SIZE" -ge "$MIN_SIZE_BYTES" ]] &&            jq -e 'type=="array"' "$TMP_JSON" >/dev/null 2>&1; then
            DOWNLOAD_OK=true
            log "api/read/: success (${SIZE} bytes)"
        else
            log "api/read/: response too small or wrong format (${SIZE} bytes)"
        fi
    fi
fi
...
if [[ "$DOWNLOAD_OK" == false ]]; then
    if [[ -f "$OUT_FILE" ]] && grep -q "redef malicious_ja4_db" "$OUT_FILE" 2>/dev/null; then
        log "All API sources unavailable — keeping previous DB"
        echo "last_status=kept_previous" > "$STATE_FILE"
        echo "last_attempt=$(date '+%Y-%m-%d %H:%M:%S')" >> "$STATE_FILE"
        exit 0
    else
        # No previous DB at all — write the embedded baseline so detection
        # has at least minimal coverage rather than an empty DB file
        log "WARN: All API sources unavailable and no previous DB — writing embedded baseline"
        write_embedded_baseline
        echo "last_status=embedded_baseline" > "$STATE_FILE"
        echo "last_attempt=$(date '+%Y-%m-%d %H:%M:%S')" >> "$STATE_FILE"
        log "Embedded baseline written — schedule retry or run updater manually when network allows"
        exit 0
    fi
fi
...
# Generate the Zeek file
{
    ...
    echo "# Source: ${API_DOWNLOAD_URL} / ${API_READ_URL}"
...
install_forwarder() {
    ...
    1)  # Wazuh agent
        if dpkg -l wazuh-agent 2>/dev/null | grep -q "^ii"; then
            ...
            # Method 1 — RH Pulsar version file (served by nginx on manager)
            manager_ver=$(curl -sf --max-time 3 \
                "http://${SIEM_HOST}/rh-pulsar-wazuh-version.txt" \
...
" 2>/dev/null; then
        ...
        # ── Deploy AR script to sensor ───────────────────────────
        # Required: AR <location>all</location> on manager sends the command
        # to this agent — the agent looks for the script locally.
        mkdir -p /var/ossec/active-response/bin
        cat > /var/ossec/active-response/bin/rh-pulsar-block.sh << 'AREOF'
#!/bin/bash
# RH Pulsar Active Response (sensor side) — blocks C2 IP via iptables
...
AREOF
        ...
        # AR restore service — survives sensor reboots
...
EOF
        ...
        ok "Wazuh Agent: AR script + restore service deployed"
