...
#  Version: 3.2.0 (Ubuntu 24.04 LTS — production + dynamic JA4 intel)
...
# ── Validate with Zeek's parser before deploying ───────────
if ! /opt/zeek/bin/zeek -a -e "@load ${TMP_OUT}" &>/dev/null; then
    log "ERROR: Generated Zeek file failed syntax check — keeping previous DB"
    echo "last_status=syntax_error" > "$STATE_FILE"
    echo "last_attempt=$(date '+%Y-%m-%d %H:%M:%S')" >> "$STATE_FILE"
    exit 3
fi
...
# Reload Zeek (zeekctl deploy is safe & idempotent)
if /opt/zeek/bin/zeekctl deploy &>/dev/null; then
    ...
    /opt/zeek/bin/zeekctl deploy &>/dev/null || true
...
EOF
    ...
    spinner_start "Running first JA4 threat intel update..."
    if systemctl start rh-pulsar-ja4-update.service >> "$LOG" 2>&1; then
        # Wait for it to finish (it's oneshot, so we can wait synchronously)
        local waited=0
        while systemctl is-active --quiet rh-pulsar-ja4-update.service && [[ $waited -lt 90 ]]; do
            sleep 1; waited=$((waited+1))
        done
        spinner_stop

        if [[ -f /var/lib/rh-pulsar/ja4-update.state ]] && \
           grep -q "last_status=success" /var/lib/rh-pulsar/ja4-update.state; then
            ...
            warn "JA4 first update incomplete — detection works with manual fingerprints only"
            warn "Will retry tomorrow at 03:00 via systemd timer"
            warn "Manual run: sudo systemctl start rh-pulsar-ja4-update.service"
        fi
    else
        spinner_stop
        warn "JA4 first update failed to start — timer will retry tomorrow"
    fi
}
